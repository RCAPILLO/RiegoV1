from tkinter import *
from ProgramarRiego import ProgramarRiego
...
def pantallaPrincipal():
    ...
    EstadoSistema=Label(miframe,text="Inactivo",font=("comic san ms",18)).place(x=320,y=100)
    Label(miframe,text="Temperatura",font=("Comic san MS",18)).place(x=20,y=140)
    EstadoSistema=Label(miframe,text="18°C",font=("comic san ms",18)).place(x=320,y=140)
    Label(miframe,text="Humedad del Campo",font=("Comic san MS",18)).place(x=20,y=180)
    EstadoSistema=Label(miframe,text="25%",font=("comic san ms",18)).place(x=320,y=180)

    #Boton para iniciar el riego o colocarlo activo
    start=Button(miframe,text="Iniciar Riego",font=("comic san MS",18),
                 command=lambda: [VentanaPrincipal.destroy(), ProgramarRiego()]
                 )
